...
parser.add_argument('--train_filename', default='./data_flist/train_shuffled.flist3', type=str,
                    help='The output filename.')
parser.add_argument('--validation_filename', default='./data_flist/validation_shuffled3.flist', type=str,
                    help='The output filename.')
...
if __name__ == "__main__":
    ...
    for dir_item in dirs:
        ...
        arz_folder = os.listdir(dir_item + "/training")
        for arz_item in arz_folder:
            arz_item = dir_item + "/training" + "/" + arz_item
...
'''
    ...
    for j in range(min(len(training_file_names),len(validation_file_names),lem(arz_file_names))):
        print(training_file_names[j])
        #print()
        #fo.write("\n".join(training_file_names[j]))
        #fo.write("\n".join(validation_file_names[j]))
        #fo.write("\n".join(arz_file_names[j]))
    fo.close()
