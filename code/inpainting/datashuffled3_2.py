...
parser.add_argument('--folder_path', default='/home/lws/training_data', type=str,
                    help='The folder path')
parser.add_argument('--train_filename', default='/home/lws/data_flist/train_shuffled_l1.flist', type=str,
                    help='The train filename.')
parser.add_argument('--validation_filename', default='/home/lws/data_flist/validation_shuffled.flist', type=str,
...
if __name__ == "__main__":
    ...
    arz_dirs = os.listdir(args.folder_path + "/arz")
    ...
    for validation_dir in validation_dirs:
        ...
        for validation_item in validation_folder:
            ...
            if (validation_item.find("mask")==-1):
                validation_file_names.append(validation_item)

    for arz_dir in arz_dirs:
        # append each file into the list file names
        arz_folder = os.listdir(args.folder_path + "/arz" + "/" + arz_dir)
        for arz_item in arz_folder:
            # modify to full path -> directory
            arz_item = args.folder_path + "/arz" + "/" + arz_dir + "/" + arz_item

            if (arz_item.find("mask")==-1):
                arz_file_names.append(arz_item)
    ...
    for j in range(min(len(training_file_names), len(validation_file_names), len(arz_file_names))):
        print(training_file_names[j])
        fo.write(arz_file_names[j]+' ')
        fo.write(training_file_names[j]+' ')
        fo.write(validation_file_names[j]+'\n')
